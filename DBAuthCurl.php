...
class DBAuthCurl{
	...
	public static function dbGet($url, $method='GET', $parameters = array()){	
		$response = self::dbAuthRequest($url, strtoupper($method), $parameters);
	 ...
	 * @ignore
	 */
	private static function dbAuthRequest($url, $method, $parameters) {
		switch ($method) {
			case 'POST':
				$body = array();
				...
				return self::dbHttp($url, 'POST', $body);
			default:
				$url = $url . '?' . http_build_query($parameters);
	
				return self::dbHttp($url, 'GET');
	 ...
	 * @ignore
	 */
	private static function dbHttp($url, $method, $postfields = NULL, $headers = array()) {
		$http_info = array();
		$http_code = '';
		$ci = curl_init();
		/* Curl settings */
